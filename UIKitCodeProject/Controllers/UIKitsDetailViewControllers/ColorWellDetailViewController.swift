//
//  ColorWellDetailViewController.swift
//  UIKitCodeProject
//
...
class ColorWellDetailViewController: UIViewController {

    private var detailView = ColorWellDetailView()
    ...
    func setupCode() {
        switch codeTag {
        case 0:
            detailView.colorWell.title = textData
        case 1:
            if let boolCheck = Bool(textData) {
                detailView.colorWell.supportsAlpha = boolCheck
            }
        default:
            break
        }
...
extension ColorWellDetailViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row <= (uikitCodeData?.UIKitFunction.count)! - 1 {
            ...
            case "colorWell.selectedColor":
                let red = UIAction(title: "red", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .red })
                let green = UIAction(title: "green", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .green })
                let blue = UIAction(title: "blue", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .blue })
                let gray = UIAction(title: "gray", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .gray })
                let white = UIAction(title: "white", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .white })
                let black = UIAction(title: "black", image: UIImage(systemName: ""), handler: { _ in self.detailView.colorWell.selectedColor = .black })
...
extension ColorWellDetailViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        
    }
}

extension ColorWellDetailViewController: UITextFieldDelegate {
